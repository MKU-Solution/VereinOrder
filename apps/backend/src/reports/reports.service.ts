...
export class ReportsService {
  ...
  async getSummary(eventId?: string) {
    ...
    for (const order of orders) {
      ...
      const orderPaid = order.payments.reduce((sum, p) => p.status === 'COMPLETED' ? sum + p.amount : sum, 0);
    ...
    const cancelledAmount = cancelledOrders.reduce((sum, o) => sum + o.totalAmount, 0);
  ...
  async getCategoriesSummary(eventId?: string) {
    ...
    for (const p of products) {
      if (!categoryMap.has(p.categoryName)) {
        categoryMap.set(p.categoryName, { name: p.categoryName, quantity: 0, revenue: 0 });
      }
  ...
  async getHourlySummary(eventId?: string) {
    ...
    const hourlyMap = new Map<string, { hour: string; count: number; revenue: number }>();
  ...
  async getSessionsSummary(eventId?: string) {
    ...
    return sessions.map(session => {
      ...
      const difference = session.closingBalance !== null && session.closingBalance !== undefined 
  ...
  async exportCsv(type: 'orders' | 'products' | 'users' | 'sessions' | 'categories', eventId?: string): Promise<string> {
    const BOM = '\uFEFF';
    const whereEvent = this.getEventFilter(eventId);

    if (type === 'products') {
      const products = await this.getProductsSummary(eventId);
      const header = ['Produkt', 'Kategorie', 'Einzelpreis (€)', 'Steuersatz (%)', 'Verkaufte Menge', 'Gesamtumsatz (€)'].join(';');
    ...
    if (type === 'categories') {
      ...
      const header = ['Kategorie', 'Verkaufte Artikel', 'Gesamtumsatz (€)'].join(';');
    ...
    if (type === 'users') {
      ...
      const header = ['Mitarbeiter', 'Rolle', 'Bestellungen', 'Bar-Einnahmen (€)', 'Karten-Einnahmen (€)', 'Gesamtumsatz (€)'].join(';');
    ...
    if (type === 'sessions') {
      ...
      const header = ['Sitzung ID', 'Mitarbeiter', 'Status', 'Startzeit', 'Endzeit', 'Startguthaben (€)', 'Barumsatz (€)', 'Soll-Bargeld (€)', 'Gezählt (€)', 'Differenz (€)'].join(';');
    ...
    if (type === 'orders') {
      ...
      const header = ['Bestell-ID', 'Bestellnummer', 'Zeitpunkt', 'Tisch / Bereich', 'Mitarbeiter', 'Status', 'Zahlungsstatus', 'Gesamtbetrag (€)', 'Bezahlt Bar (€)', 'Bezahlt Karte (€)'].join(';');
      const rows = orders.map(o => {
        const cash = o.payments.filter(p => p.method === 'CASH' && p.status === 'COMPLETED').reduce((sum, p) => sum + p.amount, 0);
        const card = o.payments.filter(p => p.method === 'CARD' && p.status === 'COMPLETED').reduce((sum, p) => sum + p.amount, 0);

...
export const RevisionDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'products' | 'staff' | 'sessions'>('overview');
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const active = res.data.find((e: any) => e.status === 'ACTIVE' || e.status === 'TEST_MODE') || res.data[0];
  ...
  const fetchData = useCallback(async (showRefreshing = false) => {
    ...
    try {
      ...
      const [sumRes, prodRes, catRes, userRes, hourRes, sessRes] = await Promise.all([
  ...
  const formatDate = (isoStr?: string) => {
    ...
    return d.toLocaleTimeString('de-AT', { hour: '2-digit', minute: '2-digit' }) + ' (' + d.toLocaleDateString('de-AT', { day: '2-digit', month: '2-digit' }) + ')';
  };

  const handleExport = async (type: 'orders' | 'products' | 'users' | 'sessions' | 'categories') => {
    setExportingType(type);
    try {
      const res = await api.get(`/reports/export/${type}?eventId=${selectedEventId}`, {
        responseType: 'blob'
      });
  ...
  if (isLoading && events.length === 0) {
    return <div className="text-center py-20 text-slate-400 animate-pulse">Lade Auswertungen...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass p-5 rounded-3xl flex items-center gap-4 relative overflow-hidden border-indigo-500/20">
          ...
          <div className="min-w-0">
            <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Gesamtumsatz</div>
            <div className="text-2xl font-black text-white truncate">{formatPrice(summary.totalAmount)}</div>
            <div className="text-xs text-slate-400 mt-0.5">{summary.orderCount} Bestellungen</div>
          </div>
        </div>
        ...
        <div className="glass p-5 rounded-3xl flex items-center gap-4 relative overflow-hidden border-emerald-500/20">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Bar-Umsatz</div>
            <div className="text-2xl font-black text-emerald-400 truncate">{formatPrice(summary.cashRevenue)}</div>
        ...
        <div className="glass p-5 rounded-3xl flex items-center gap-4 relative overflow-hidden border-amber-500/20">
          ...
          <div className="min-w-0">
            <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Offene Tische</div>
            <div className="text-2xl font-black text-amber-300 truncate">{formatPrice(summary.openAmount)}</div>
        ...
        <div className="glass p-5 rounded-3xl flex items-center gap-4 relative overflow-hidden border-rose-500/20">
          ...
          <div className="min-w-0">
            <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Stornos</div>
            <div className="text-2xl font-black text-rose-400 truncate">{formatPrice(summary.cancelledAmount)}</div>
            <div className="text-xs text-slate-400 mt-0.5">{summary.cancelledCount} storniert</div>
          </div>
        </div>
      </div>
      ...
      <div className="glass p-4 rounded-3xl flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {exportingType === 'orders' ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <ShoppingBag className="w-3.5 h-3.5 text-indigo-400" />}
            Bestellungen
          </button>
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-xs font-semibold text-slate-200 border border-slate-700/60 transition-colors flex items-center gap-1.5"
          >
            {exportingType === 'products' ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Package className="w-3.5 h-3.5 text-emerald-400" />}
            Artikelverkäufe
          </button>
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-xs font-semibold text-slate-200 border border-slate-700/60 transition-colors flex items-center gap-1.5"
          >
            {exportingType === 'users' ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Users className="w-3.5 h-3.5 text-blue-400" />}
            Mitarbeiter
          </button>
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-xs font-semibold text-slate-200 border border-slate-700/60 transition-colors flex items-center gap-1.5"
          >
            {exportingType === 'sessions' ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Wallet className="w-3.5 h-3.5 text-amber-400" />}
            Kassenabschlüsse
          </button>
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-xs font-semibold text-slate-200 border border-slate-700/60 transition-colors flex items-center gap-1.5"
          >
            {exportingType === 'categories' ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Layers className="w-3.5 h-3.5 text-purple-400" />}
            Kategorien
          </button>
        </div>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="glass p-6 rounded-3xl">
            ...
            {hourly.length === 0 ? (
              <p className="text-slate-500 text-center py-8">Noch keine Bestellungen im Verlauf vorhanden.</p>
            ) : (
              <div className="space-y-3">
                {hourly.map((h) => {
                  const width = Math.max((h.revenue / maxHourlyRevenue) * 100, 3);
                  return (
                    <div key={h.hour} className="flex items-center gap-3">
                      <div className="w-16 text-xs font-bold text-slate-400 flex-shrink-0">{h.hour}</div>
                      <div className="flex-1 h-8 bg-slate-900/60 rounded-xl overflow-hidden relative border border-slate-800">
                        ...
                        <div className="absolute inset-0 flex items-center justify-between px-3 text-xs">
                          <span className="font-semibold text-slate-200">{h.count} Bestellungen</span>
                          <span className="font-bold text-white">{formatPrice(h.revenue)}</span>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="glass p-6 rounded-3xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {categories.map((c) => (
                <div key={c.name} className="bg-slate-900/60 border border-slate-800 p-4 rounded-2xl">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-semibold text-slate-200">{c.name}</span>
                    <span className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded-lg">{c.quantity} Stk.</span>
                  </div>
                  <div className="text-xl font-bold text-white">{formatPrice(c.revenue)}</div>
                </div>
              ))}
              {categories.length === 0 && <p className="text-slate-500">Keine Daten verfügbar.</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'products' && (
        <div className="glass p-6 rounded-3xl space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">{products.length} verschiedene Produkte</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {products.map((p, idx) => {
                  const width = Math.max((p.revenue / maxProductRevenue) * 100, 3);
                  return (
                    <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-3 font-medium text-slate-200">
                        <span className="text-slate-500 font-bold mr-2">#{idx + 1}</span>
                        {p.name}
                      </td>
                      <td className="py-3 text-slate-400">
                        <span className="bg-slate-800/80 px-2 py-0.5 rounded-md text-xs">{p.categoryName}</span>
                      </td>
                      <td className="py-3 text-right text-slate-300">{formatPrice(p.price)}</td>
                      <td className="py-3 text-right font-bold text-slate-100">{p.quantity}x</td>
                      <td className="py-3 text-right font-bold text-emerald-400">{formatPrice(p.revenue)}</td>
                      <td className="py-3">
                        <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-500/50 rounded-full" style={{ width: `${width}%` }} />
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {activeTab === 'staff' && (
        <div className="glass p-6 rounded-3xl space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">{users.length} aktive Mitarbeiter</span>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="text-slate-400 border-b border-slate-800">
                  ...
                  <th className="pb-3 font-semibold text-right">Bestellungen</th>
                  <th className="pb-3 font-semibold text-right">Bar eingenommen</th>
                  <th className="pb-3 font-semibold text-right">Karte eingenommen</th>
                  <th className="pb-3 font-semibold text-right">Gesamtumsatz</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 text-right font-medium text-slate-300">{u.orderCount}</td>
                    <td className="py-3 text-right font-bold text-emerald-400">{formatPrice(u.cashRevenue)}</td>
                    <td className="py-3 text-right font-bold text-blue-400">{formatPrice(u.cardRevenue)}</td>
                    <td className="py-3 text-right font-black text-white">{formatPrice(u.revenue)}</td>
                  </tr>
                ))}
                {users.length === 0 && (
                  <tr>
      ...
      {activeTab === 'sessions' && (
        <div className="glass p-6 rounded-3xl space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">{sessions.length} erfasste Sitzungen</span>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="text-slate-400 border-b border-slate-800">
                  ...
                  <th className="pb-3 font-semibold text-right">Gezählt (Ist)</th>
                ...
                {sessions.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-3 font-bold text-slate-100">{s.username}</td>
                      <td className="py-3">
                        <span
                      ...
                      <td className="py-3 text-xs text-slate-400">
                        {formatDate(s.startTime)} {s.endTime ? `bis ${formatDate(s.endTime)}` : ''}
                      </td>
                      <td className="py-3 text-right text-slate-300">{formatPrice(s.startingBalance)}</td>
                      <td className="py-3 text-right font-medium text-emerald-400">+{formatPrice(s.cashSales)}</td>
                      <td className="py-3 text-right font-bold text-slate-100">{formatPrice(s.expectedCash)}</td>

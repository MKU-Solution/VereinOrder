...
export const StationView = () => {
  ...
  const handleSetAvailability = async (productId: string, availability: 'AVAILABLE' | 'LOW_STOCK' | 'OUT_OF_STOCK') => {
    setStationProducts(prev => prev.map(p => p.id === productId ? { ...p, availability } : p));
    try {
      ...
      <div className="flex items-center justify-between glass p-4 rounded-2xl sticky top-24 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {showStockPanel ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </button>
      ...
      {showStockPanel && (
        <div className="glass p-5 rounded-3xl animate-slide-up border-amber-500/30 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">Änderungen werden sofort live an alle Kellner übertragen</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {stationProducts.map((p) => (
              <div key={p.id} className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-2xl space-y-2.5">
                <div className="flex justify-between items-start">
                  <span className="font-bold text-slate-200 text-base leading-tight">{p.name}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-md font-bold ${
                    p.availability === 'AVAILABLE' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                    p.availability === 'LOW_STOCK' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' :
                    'bg-rose-500/20 text-rose-400 border border-rose-500/30'
                  }`}>
                    {p.availability === 'AVAILABLE' ? 'Verfügbar' : p.availability === 'LOW_STOCK' ? 'Knapp' : 'Ausverkauft'}
                  </span>
                </div>
            ...
            {stationProducts.length === 0 && (
              <p className="text-slate-500 text-sm col-span-3 py-2">Dieser Station sind noch keine Produkte zugeordnet.</p>
            )}
          </div>
        </div>
      )}

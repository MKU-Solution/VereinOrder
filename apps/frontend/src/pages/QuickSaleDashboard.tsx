...
export const QuickSaleDashboard = () => {
  ...
  // Bestandsänderungen sind in der Ein-Tipp-Kasse sofort sichtbar; eine
  // bereits gewählte Position bleibt dabei bewusst im Warenkorb und wird
  // erst beim bewussten Absenden fachlich geprüft.
  useEffect(() => {
    ...
    // stehen, bis der nächste "loadContext"-Lauf neu lädt.
  ...
  const submitSale = async (paymentMethod: "CASH" | "CARD") => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.code === "INVENTORY_INSUFFICIENT"
          ? "Der Bestand hat sich während der Eingabe geändert. Der Warenkorb bleibt unverändert: Bitte Menge und Verfügbarkeit prüfen."
          : requestError.response?.data?.message ||

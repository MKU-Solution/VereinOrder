  ...
  const handleCancelOrder = async () => {
    ...
    try {
      await api.post(`/orders/${order.id}/cancel`, {
        reason: "Storniert durch Kellner",
      });
      onRefresh();
      onClose();
    } catch (err: any) {
      ...
      <div className="bg-slate-800 rounded-3xl w-full max-w-lg border border-slate-700/50 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-slate-300 border-b border-slate-700/50 pb-2">
              Positionen
            </h3>
            {order.items.map((item: any) => (
              <div
                key={item.id}
                className={`flex justify-between items-center p-3 rounded-xl ${item.status === "CANCELLED" ? "opacity-50 bg-slate-900/50" : "bg-slate-900/50"}`}
              >
